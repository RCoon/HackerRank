 ...
 * which takes two arguments - the head of the linked list and the position of
 * the node from the tail. positionFromTail will be at least 0 and less than the
 * number of nodes in the list. You should NOT read any input from
 * stdin/console.
 * 
 * Constraints:
...
public class GetNodeValue {
	...
	int GetNode(Node head,int n) {
	    ...
	    runner = head;
	    int nodesFromHead = numNodes - n;
	    while (nodesFromHead > 0) {
	        runner = runner.next;
	        nodesFromHead--;
	    }
	    return runner.data;
