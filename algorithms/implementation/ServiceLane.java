...
public class ServiceLane {
	...
	public static void main(String[] args) {
        ...
        int fwLength = sc.nextInt();
        int numCases = sc.nextInt();
        
        int[] fwWidthArray = new int[fwLength];
        for (int i = 0; i < fwLength; i++) {
            fwWidthArray[i] = sc.nextInt();
        }
        ...
        for (int j = 0; j < numCases; j++) {
            ...
            System.out.println(smallestWidth(beginIndex, endIndex, fwWidthArray));
        }
        sc.close();
    }
    
    static int smallestWidth(int startIndex, int finishIndex, int[] array) {
        
        int leastWidth = 3;
        
        for (int i = startIndex; i <= finishIndex; i++) {
            if (array[i] < leastWidth) {
                leastWidth = array[i];
            }
        }
        return leastWidth;
